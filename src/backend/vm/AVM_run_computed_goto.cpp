...
#define DATA_MEMBER_CAL_DATA(opcode, data1, data2)                             \
	do_##opcode : {                                                            \
		...
		tempAllocateArea[0] =                                                  \
		    parent->member->data[get_u32(bytecodes, ip)];                      \
		tempAllocateArea[1] = data2[get_u32(bytecodes, ip)];                   \
...
#define DATA_MEMBER_CAL_DATA_MEMBER(opcode, data1, data2)                      \
	do_##opcode : {                                                            \
		...
		tempAllocateArea[0] =                                                  \
		    parent1->member->data[get_u32(bytecodes, ip)];                     \
		...
		tempAllocateArea[1] =                                                  \
		    parent2->member->data[get_u32(bytecodes, ip)];                     \
		if (!fastOperate<2>(operatorTable[tablePos]))                          \
			goto resumeCallFrame;                                              \
		DISPATCH();                                                            \
	}
	...
	try {
		...
		if (!dispatchInitialized) {
			...
			dispatchTable[Autolang::Opcode::REMOVE_FINALLY] =
			    &&do_REMOVE_FINALLY;
			dispatchTable[Autolang::Opcode::END_FINALLY] =
			    &&do_END_FINALLY;
			...
			dispatchTable[Autolang::Opcode::CREATE_ARRAY_OBJECT] =
			    &&do_CREATE_ARRAY_OBJECT;
...
#define DISPATCH()                                                             \
	...
	do_LOAD_GLOBAL: {
		AObject *obj = globalVariables[get_u32(bytecodes, ip)];
		if (!obj) {
			notifier->throwException("Variable is used before initialization");
			goto resumeCallFrame;
		}
		...
		}
		{
			...
			for (auto &[name, index] : clazz->memberMap) {
				if (index != memberIndex)
					continue;
				notifier->throwException("Member '" + std::string(name) +
				                         "' at class '" + className +
				                         "' is uninitialized ");
				goto resumeCallFrame;
			}
	...
	do_ADD_TRY_BLOCK: {
		data.allHandlers.push_back(
		    {HandlerEntry::CATCH, get_u32(bytecodes, ip)});
		DISPATCH();
	}
	...
	do_ADD_FINALLY_BLOCK: {
		data.allHandlers.push_back(
		    {HandlerEntry::FINALLY, get_u32(bytecodes, ip)});
		DISPATCH();
	}
	...
	do_REMOVE_FINALLY: {
		assert(data.allHandlers.size() >
		       currentCallFrame->handlerIndex);
		data.allHandlers.pop_back();
		data.allFinallyStates.push_back({FinallyState::NORMAL, nullptr});
		DISPATCH();
	}
	...
	do_GLOBAL_CAL_CONST_JUMP: {
		...
		auto resObj = operatorTable[tablePos](*notifier, tempAllocateArea, size);
	...
	do_GLOBAL_CAL_GLOBAL_JUMP: {
		uint8_t tablePos = bytecodes[ip++];
		auto obj1 = globalVariables[get_u32(bytecodes, ip)];
		auto obj2 = globalVariables[get_u32(bytecodes, ip)];
		if (!obj1 || !obj2) {
			notifier->throwException("Variable is used before initialization");
			goto resumeCallFrame;
		}
