...
#define optimizeNode(token, func) \
	...
	ConstValueNode *BinaryNode::calculate(in_func)
	{
		...
		try
		{
			...
			optimizeNode(AND, bitwise_and)
			optimizeNode(OR, bitwise_or)
					...
					value->classId != AutoLang::DefaultClass::nullClassId)
				{
					...
					if (node->declaration->mustInferenceNullable) {
						...
						switch (valueNode->kind) {
							case VAR:
							case GET_PROP: {
								node->declaration->nullable = static_cast<AccessNode*>(value)->nullable;
								break;
							}
							case CALL: {
								node->declaration->nullable = static_cast<CallNode*>(value)->nullable;
								break;
							}
							case UNKNOW: {
								valueNode = static_cast<UnknowNode*>(value)->correctNode;
								goto back;
							}
						}
	...
	bool CallNode::match(in_func, MatchOverload &match, std::vector<uint32_t> &functions, int &i)
	{
		...
		for (; i < functions.size(); ++i)
		{
			...
			for (int j = 0; j < arguments.size(); ++j)
			{
				...
				// printDebug(compile.classes[inputClassId].name + " and " + compile.classes[funcArgClassId].name);
	...
	void ReturnNode::optimize(in_func)
	{
		...
		if (value) {
			...
			if (value->classId == func->returnId) return;
