...
loadListArgument(in_func, size_t &i,
	...
	if (!nextToken(&token, context.tokens, i)) {
		...
		throw ParserError(0, "Expected closing bracket but reached end of file");
	}
			...
			if (argName != 0) {
				for (auto prevName : *argumentNames) {
					if (prevName != 0 && prevName == argName) {
						throw ParserError(tok->line,
						                  "Duplicate argument '" +
						                      std::string(context.lexerString[argName]) +
						                      "' in function call\n"
...
expectedCloseBracket:
	...
	for (auto *node : nodes)
		ExprNode::deleteNode(node);
	throw ParserError(token->line,
	                  "Expected closing bracket but reached end of file");
}
...
HasClassIdNode *inferenceNodeFromLBrace(in_func, size_t &i,
	...
	switch (token->type) {
		...
		default: {
			...
			if (canBeNodeType == NodeType::UNKNOW &&
			    hasItIdentifierAtCurrentBraceLevel(context, context.tokens, i)) {
				--i;
				return loadClosure(in_data, i);
			}
