...
CreateFuncNode* loadFunc(in_func, size_t& i) {
	...
	if (isStatic) context.modifierflags &= ~ModifierFlags::STATIC;
	...
	CreateFuncNode* node = context.newFunctions.push(
		firstLine, context.currentClassId, name, std::move(returnClass), returnNullable,
	...
	if (!node->returnClass.empty()) {
		...
		if (!hasReturn)
			throw ParserError(firstLine, "Function " + func->name + " didn't declare return");
	}
...
ReturnNode* loadReturn(in_func, size_t& i) {
	...
	if (!nextTokenSameLine(&token, context.tokens, i, token->line)) {
		--i;
		auto value = context.getCurrentFunctionInfo(in_data)->isConstructor ? new VarNode(firstLine, context.getCurrentClassInfo(in_data)->declarationThis, false, false) : nullptr;
		return context.returnPool.push(firstLine, context.currentFunctionId, value);
	}
	if (context.getCurrentFunctionInfo(in_data)->isConstructor)
		throw ParserError(token->line, "Cannot return value in constructor");
	return context.returnPool.push(firstLine, context.currentFunctionId, loadExpression(in_data, 0, i));
}
