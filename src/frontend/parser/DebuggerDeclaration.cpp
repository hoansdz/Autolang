...
createNode:;
	...
	auto node = context.makeDeclarationNode(
	    in_data, token->line, false, declarationName, classDeclaration, isVal,
	    isGlobal, nullable, isInFunction);
	...
	if (isStatic) {
		auto setNode =
		    (isLateinit && autogeneratedValue)
		        ? nullptr
		        : context.setValuePool.push(
		              firstLine,
		              context.varPool.push(firstLine, node, false, true), value,
		              true);
		// Function
		...
		if (context.currentFunctionId != context.mainFunctionId) {
			...
			funcInfo->staticDeclaration.push_back({node, value});
		} else { // Class
